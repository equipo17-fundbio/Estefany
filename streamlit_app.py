...
def doctor_panel():
    ...
    La rehabilitación física, especialmente en pacientes con discapacidades motoras o en recuperación de lesiones, presenta un desafío significativo debido a la necesidad de monitoreo constante. La electromiografía (EMG) ha emergido como herramienta clave para evaluar la actividad muscular, pero los sistemas actuales carecen de seguimiento continuo y accesible. Se propone el desarrollo de una plataforma web interactiva que integre redes neuronales para clasificar gestos musculares a partir de señales EMG, mejorando el monitoreo en tiempo real y optimizando los tratamientos de rehabilitación.
...
def logout():
    ...
    st.success("Has cerrado sesión exitosamente. Te redirigiré a la página de inicio de sesión.")
    time.sleep(2)
...
def main():
    ...
    menu = st.sidebar.selectbox("Seleccione una opción", ["Panel del Doctor", "Gestión de Pacientes", "Perfil del Paciente", "Historial del Paciente", "Configuración de Cuenta", "Cerrar sesión"])
