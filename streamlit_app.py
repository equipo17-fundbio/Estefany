...
import numpy as np
from tensorflow.keras.models import load_model  # Para cargar el modelo H5
...
def patient_profile():
    ...
    if patient == "Ninguno":
        ...
        # Subir modelo H5
        st.markdown("### Subir Modelo de Clasificación (.h5)")
        model_file = st.file_uploader("Subir archivo del modelo (.h5)", type="h5")
        
        if model_file:
            st.success("📁 Modelo cargado exitosamente.")
            model = load_model(model_file)  # Cargar el modelo .h5
            st.write("Modelo cargado y listo para predecir.")
            ...
            if uploaded_file:
                ...
                # Procesar archivo y predecir
                st.markdown("### Resultados del modelo:")
                try:
                    # Simular procesamiento real del archivo TXT
                    # Aquí debes implementar tu lógica para procesar el archivo de texto
                    # Ejemplo: Convertir texto en matriz de entrada para el modelo
                    input_data = np.random.rand(1, 100, 10)  # Cambiar según la estructura de entrada del LSTM
                    
                    # Predecir similitud
                    prediction = model.predict(input_data)
                    similarity_percentage = round(prediction[0][0] * 100, 2)
                    st.info(f"El paciente realizó un **{similarity_percentage}%** del gesto esperado.")
                except Exception as e:
                    st.error(f"Error al procesar los datos o predecir: {e}")
